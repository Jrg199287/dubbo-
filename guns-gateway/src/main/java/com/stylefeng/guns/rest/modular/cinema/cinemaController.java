...
import com.stylefeng.guns.api.user.order.vo.OrderServiceAPI;
...
public class cinemaController {
    ...
    @Reference(interfaceClass = com.stylefeng.guns.api.user.cinema.cinemaServiceAPI.class,
                    connections = 10,cache = "lru",check = false)
    private com.stylefeng.guns.api.user.cinema.cinemaServiceAPI cinemaServiceAPI;
    @Reference(interfaceClass = com.stylefeng.guns.api.user.order.vo.OrderServiceAPI.class,
            connections = 10,cache = "lru",check = false)
    ...
    public ResponseVO getCinemas(CinemaQueryVO cinemaQueryVO){
        try{
            // 按照五个条件进行筛选
            CinemaVOInfo cinemas = cinemaServiceAPI.getCinemas(cinemaQueryVO);
            // 判断是否有满足条件的影院
            if(cinemas.getRecords() == null || cinemas.getRecords().size()==0){
                return ResponseVO.success("没有影院可查");
            }else{
                return ResponseVO.success(cinemas.getCurrent(),(int)cinemas.getSize(),"",cinemas.getRecords());
            }
